"""晨報系統主入口：組內容 -> 寫看板資料（含歷史紀錄）-> 推 Discord 連結 -> 更新記憶。

用法：
    python -m morning_report.generate_report

本機測試（不呼叫 Groq API、不推播 Discord，只驗證資料流程）：
...
def main() -> None:
    ...
    print(f"[generate_report] 產生內容完成：{report.ticker_message[:50]}...")

    write_report(today, report.to_dict())
    print(f"[generate_report] 看板資料已寫入 {config.REPORT_DATA_PATH}（含歷史紀錄）")

    send_morning_link(_build_report_url(today))
