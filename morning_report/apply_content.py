...
Routine 自己（用 Claude 寫作，不呼叫任何 API）已經把今天的晨報內容寫進
morning_report/state/draft_content.json（欄位對應 ReportContent），這支腳本負責把它跟
pending_request.json 合併、寫進看板資料（含歷史紀錄）、更新習慣追蹤，最後 commit + push。
...
from . import config
from .board_writer import write_report
...
def main() -> None:
    ...
    report = ReportContent(
        ...
        schedule_summary=draft.get("schedule_summary", ""),
    ...
    print(f"[apply_content] 看板資料已寫入（含歷史紀錄），日期 {today.isoformat()}")
